...
## engine/src/threepowers/characterize.py (11)
...
| 80 | echoed-message | 3PWR-FR-053 | `"**Status**: Reconstructed (3PWR-FR-053)",` |
| 86 | echoed-message | 3PWR-FR-004 | `"## Non-Goals *(mandatory — 3PWR-FR-004)*",` |
| 90 | echoed-message | 3PWR-FR-051 | `"(3PWR-FR-051).",` |
| 117 | docstring | 3PWR-FR-053 | `Generated by `3pwr characterize` (3PWR-FR-053). Each test references its reconstructed` |
| 141 | docstring | 3PWR-NFR-015 | `except Exception as exc:  # legacy import deps absent → surface as skip (3PWR-NFR-015)` |
| 175 | docstring | 3PWR-FR-053 | `"""Reconstruct a spec stub + characterization tests for one file ``module_path`` (3PWR-FR-053).` |
...
| 161 | echoed-message | SRCX-FR-005 | `"(SRCX-FR-005).",` |
| 176 | docstring | SRCX-FR-006 | `"""The single ``implement.md`` record — every phase, every change, deterministic order (SRCX-FR-006).` |
| 178 | docstring | PHASE-FR-012 | ```phases`` are the per-phase results in artifact order (as collected — PHASE-FR-012); each phase` |
| 180 | docstring | SRCX-FR-005 | `set is always listed, so the record links a superset of the stage's change set (SRCX-FR-005)."""` |
| 186 | echoed-message | SRCX-FR-005 | `"neither relocates nor duplicates them (SRCX-FR-005).",` |
| 192 | comment | SRCX-FR-006 | `for ph in phases:  # deterministic artifact order, as collected (SRCX-FR-006)` |
| 219 | docstring | SRCX-FR-004 | `"""Write the ``oracle.md`` / ``implement.md`` record flat into the feature folder (SRCX-FR-004/005).` |
| 222 | docstring | SRCX-FR-013 | `the signed ``run``/``stage`` entry records it (SRCX-FR-013) and the completion gate can hold. For a` |
| 224 | docstring | SRCX-NFR-006 | `deterministic order (SRCX-NFR-006)."""` |
...
## engine/src/threepowers/frame.py (51)
...
| 111 | echoed-message | STEER-FR-013 | `guidance: str = ""  # the gate action guidance, shown while paused (STEER-FR-013)` |
...
## engine/src/threepowers/gates.py (61)
...
| 308 | echoed-message | 3PWR-FR-052 | `findings=["no spec resolved — brownfield report-only/diff-scope (3PWR-FR-052)"],` |
...
## engine/src/threepowers/gitflow.py (49)
...
| 34 | echoed-message | GITX-FR-007 | `GATE_CLEAN_START = "git_clean_start"  # the clean-start guard (GITX-FR-007)` |
| 35 | echoed-message | GITX-FR-010 | `GATE_STAGE_COMMIT = "git_stage_commit"  # the mandatory per-stage commit (GITX-FR-010)` |
| 36 | echoed-message | GITX-FR-016 | `GATE_RUN_BRANCH = "git_run_branch"  # the branch-isolation guard at stage boundaries (GITX-FR-016)` |
...
## engine/src/threepowers/keys.py (20)
...
| 169 | echoed-message | HARDN-FR-006 | `raise ExternalSignerError("external signer command is empty (HARDN-FR-006)")` |
| 175 | echoed-message | HARDN-FR-006 | `"back to a software key (HARDN-FR-006)"` |
| 181 | echoed-message | HARDN-FR-006 | `"refusing to fall back to a software key (HARDN-FR-006)"` |
| 188 | echoed-message | HARDN-FR-006 | `"of stdin on stdout (HARDN-FR-006)"` |
| 194 | echoed-message | HARDN-FR-006 | `"wrong key (HARDN-FR-006)"` |
| 208 | echoed-message | HARDN-FR-006 | `"install the external signer's public key there first (HARDN-FR-006)"` |
...
| 278 | echoed-message | 3PWR-NFR-005 | `"repository (3PWR-NFR-005)"` |
...
| 313 | echoed-message | 3PWR-NFR-005 | `"The private key must live OUTSIDE the repository (3PWR-NFR-005)."` |
| 322 | docstring | HARDN-FR-006 | `private seed is never readable by the engine (HARDN-FR-006). A configured-but-broken` |

## engine/src/threepowers/ledger.py (10)
...
| 40 | echoed-message | HARDN-FR-004 | `"key_rotation",  # signer succession, authored by the OUTGOING key (HARDN-FR-004)` |
| 41 | echoed-message | HARDN-FR-005 | `"anchor",  # local receipt of an external ledger-head anchor (HARDN-FR-005)` |
...
## engine/src/threepowers/lifecycle.py (12)
...
| 42 | echoed-message | 3PWR-FR-011 | `pending_gate: str = ""  # a `3pwr run` orchestration paused at this human gate (3PWR-FR-011/019)` |
...
## engine/src/threepowers/observe.py (8)
...
| 37 | echoed-message | 3PWR-FR-054 | `"# Feedback intents — production lessons routed back to the legislature (3PWR-FR-054)\n\n"` |
...
## engine/src/threepowers/orchestrate.py (69)
...
| 81 | echoed-message | RUNLIVE-FR-002 | `""  # a human-readable failure detail (e.g. the missing-artifact message, RUNLIVE-FR-002)` |
| 97 | comment | RUNX-FR-010 | `# "" = a dispatch/execution failure (not a gate verdict) — the honest-diagnostics split (RUNX-FR-010/011)` |
| 102 | echoed-message | RUNLIVE-FR-002 | `""  # a human-readable failure detail (e.g. the missing-artifact message, RUNLIVE-FR-002)` |
| 107 | docstring | RUNX-FR-010 | `"""True only when a failure carries a real deterministic-gate ``fail`` verdict (RUNX-FR-010)."""` |
| 112 | docstring | RUNLIVE-FR-002 | `"""True when the failure was a stage producing no declared artifact (RUNLIVE-FR-002)."""` |
| 142 | docstring | RUNLIVE-FR-010 | `"""The last action step committed as a checkpoint for ``spec_id`` (RUNLIVE-FR-010), else ''.` |
| 157 | docstring | AUTOX-FR-010 | `"""The last action step recorded COMPLETE for ``spec_id``, else '' (AUTOX-FR-010).` |
| 160 | docstring | RUNLIVE-FR-010 | `(RUNLIVE-FR-010) or the lightweight ``stage`` completion record written at stage success even with` |
| 174 | docstring | RUNLIVE-FR-010 | `"""Where a ``--resume`` should re-enter the lifecycle (RUNLIVE-FR-010, AUTOX-FR-010).` |
| 174 | docstring | AUTOX-FR-010 | `"""Where a ``--resume`` should re-enter the lifecycle (RUNLIVE-FR-010, AUTOX-FR-010).` |
| 190 | docstring | RUNX-FR-007 | `including) the next gate. Used to record one per-stage dispatch provenance entry (RUNX-FR-007), so a` |
| 192 | docstring | RUNX-FR-004 | `segment (no already-completed stage is re-recorded, mirroring RUNX-FR-004)."""` |
| 197 | docstring | RUNX-FR-007 | `"""The action steps from ``start_index`` up to (not including) the next gate (RUNX-FR-007, RUNLIVE-FR-010).` |
| 197 | docstring | RUNLIVE-FR-010 | `"""The action steps from ``start_index`` up to (not including) the next gate (RUNX-FR-007, RUNLIVE-FR-010).` |
| 220 | comment | STEER-FR-013 | `# moment it happened (STEER-FR-013) — replaying the batched history here would report it twice.` |
| 241 | comment | RUNLIVE-FR-002 | `# (gate-red vs a dispatch/artifact failure — RUNLIVE-FR-002, RUNX-FR-010/011).` |
| 241 | comment | RUNX-FR-010 | `# (gate-red vs a dispatch/artifact failure — RUNLIVE-FR-002, RUNX-FR-010/011).` |
| 297 | comment | CLIUX-NFR-004 | `# Event glyphs, with an ASCII fallback for a stream that cannot encode the Unicode marks (CLIUX-NFR-004).` |
| 326 | docstring | CLIUX-FR-008 | `so the same "you are here" view reads consistently live and in ``--status`` (CLIUX-FR-008/012).` |
| 344 | docstring | GDIAG-FR-001 | `"""The first non-empty findings line of a failed gate — the line the user acts on (GDIAG-FR-001).` |
| 356 | docstring | GDIAG-FR-001 | `"""The structured gates-failed summary for a gate-red event (GDIAG-FR-001/006).` |
| 387 | docstring | CLIUX-FR-009 | `"""Human-readable one-liner for a streamed event (CLIUX-FR-009).` |
| 390 | docstring | CLIUX-FR-009 | `verdict, a prominent paused gate, a red failure — distinct at a glance (CLIUX-FR-009). With no` |
| 407 | comment | RUNX-FR-010 | `# failure is reported distinctly and names the stage reached (RUNX-FR-010/011, RUNLIVE-FR-002).` |
| 407 | comment | RUNLIVE-FR-002 | `# failure is reported distinctly and names the stage reached (RUNX-FR-010/011, RUNLIVE-FR-002).` |
| 419 | comment | SRCX-FR-014 | `# The SRCX completion gate blocked the stage (SRCX-FR-014/015) — named, actionable.` |
| 441 | docstring | CLIUX-FR-008 | `with the running step alongside it (CLIUX-FR-008/009)."""` |
| 449 | docstring | STEER-FR-012 | `dispatched agent's stdout print ABOVE it into ordinary, fully scrollable history (STEER-FR-012/013,` |
| 450 | docstring | CLIUX-FR-008 | `advancing CLIUX-FR-008/009's single in-place line). Off a TTY (pipe / ``--json``), under` |
| 452 | docstring | STEER-FR-015 | ```format_event`` log with no ``\\r`` in-place redraws and no ANSI/control codes (STEER-FR-015,` |
| 453 | docstring | CLIUX-FR-011 | `CLIUX-FR-011). The bar is rendered by ``rich`` behind the frame API (TRIX-FR-003/004; the` |
| 453 | docstring | TRIX-FR-003 | `CLIUX-FR-011). The bar is rendered by ``rich`` behind the frame API (TRIX-FR-003/004; the` |
| 456 | docstring | CLIUX-FR-011 | ```THREEPOWERS_FORCE_COLOR``, so a captured/piped run never carries escapes (CLIUX-FR-011)."""` |
| 474 | comment | CLIUX-FR-011 | `# run never carries escapes (CLIUX-FR-011).` |
| 477 | comment | STEER-FR-012 | `# The live bar (STEER-FR-012) — only when the terminal can carry it (STEER-FR-015);` |
| 477 | comment | STEER-FR-015 | `# The live bar (STEER-FR-012) — only when the terminal can carry it (STEER-FR-015);` |
| 486 | docstring | STEER-FR-016 | `"""Tear the live bar down — last state left on screen, cursor restored (STEER-FR-016).` |
| 493 | docstring | RUNID-FR-003 | `"""Adopt the run's resolved identity after construction (RUNID-FR-003).` |
| 496 | docstring | RUNID-FR-001 | `(the folder's NNN, RUNID-FR-001) arrives late; the live bar's title and its pause/resume` |
| 504 | docstring | STEER-FR-012 | `dispatch produces an event, so the run never looks frozen (STEER-FR-012/013). A no-op off a` |
| 516 | docstring | STEER-FR-012 | `open (scrollback keeps the whole conversation, STEER-FR-012), plain otherwise."""` |
| 526 | docstring | STEER-FR-012 | `above the live bar in real time instead of clobbering it (STEER-FR-012/013)."""` |
| 534 | comment | STEER-FR-012 | `# reflects the current state (STEER-FR-012/013).` |
| 546 | comment | STEER-FR-016 | `# lines so the follow-up guidance prints in normal flow (STEER-FR-016).` |
| 549 | comment | STEER-FR-015 | `# The plain streamed event log — off a TTY, and the degraded TTY path (STEER-FR-015):` |
| 557 | docstring | STEER-FR-012 | `through the tracker (STEER-FR-012) — above the live bar on a capable TTY, plain otherwise.` |
| 580 | comment | GATEPIPE-FR-004 | `# Tool-output noise the rendered gate view suppresses unless verbose (GATEPIPE-FR-004).` |
| 584 | comment | GATEPIPE-FR-003 | `# A failed gate's panel shows at most this many meaningful error lines (GATEPIPE-FR-003).` |
| 589 | docstring | GATEPIPE-FR-004 | `"""Flatten gate findings into rendered lines, filtering noise unless ``verbose`` (GATEPIPE-FR-004).` |
| 593 | docstring | GATEPIPE-NFR-001 | `only — the machine-readable verdict is never filtered (GATEPIPE-NFR-001)."""` |
| 610 | docstring | GATEPIPE-FR-001 | `"""One gate's pipeline row state — running until its finish event lands (GATEPIPE-FR-001)."""` |
| 620 | docstring | GATEPIPE-FR-001 | `"""The per-gate pipeline view of a gate run (GATEPIPE-FR-001/002).` |
| 627 | docstring | GATEPIPE-FR-002 | `line per *finished* gate, no in-place updates (GATEPIPE-FR-002). A ``--json`` run never` |
| 629 | docstring | GATEPIPE-NFR-001 | `enters the verdict (GATEPIPE-NFR-001)."""` |
| 694 | docstring | GATEPIPE-FR-002 | `"""Show ``gate`` as running (live mode); plain mode waits for the finish (GATEPIPE-FR-002)."""` |
| 727 | comment | GATEPIPE-FR-005 | `# (GATEPIPE-FR-005); pass/fail keep the shared status vocabulary (CLIUX-FR-005).` |
| 727 | comment | CLIUX-FR-005 | `# (GATEPIPE-FR-005); pass/fail keep the shared status vocabulary (CLIUX-FR-005).` |
| 741 | docstring | GATEPIPE-FR-001 | `"""The live region's renderable: one three-column grid row per gate (GATEPIPE-FR-001)."""` |
| 761 | docstring | GATEPIPE-FR-003 | `with a truncation note, then the configured auto-fix hint when present (GATEPIPE-FR-003/004).` |
| 783 | docstring | GATEPIPE-FR-003 | `plain indented text otherwise (GATEPIPE-FR-003)."""` |
| 818 | docstring | GATEPIPE-FR-003 | `"""The post-run failure surface: one panel per FAILED gate of ``verdict`` (GATEPIPE-FR-003).` |
...
## engine/src/threepowers/runpreflight.py (25)
...
| 185 | echoed-message | GITX-FR-002 | `return Prereq(name, False, "install git — a run requires version control (GITX-FR-002)")` |
| 190 | echoed-message | GITX-FR-002 | `name, False, "run `git init` — the target is not inside a git repository (GITX-FR-002)"` |
| 196 | docstring | AUTOX-FR-001 | `well-formed), never trusted silently (AUTOX-FR-001)."""` |
| 243 | docstring | AUTOX-FR-002 | `check set (AUTOX-FR-002): ``3pwr init``'s readiness, the standalone ``3pwr ready``, and the run's` |
| 247 | docstring | AUTOX-FR-005 | `(AUTOX-FR-005): signing key → working git repository (GITX-FR-002) → coder agent (roles + CLI)` |
| 247 | docstring | GITX-FR-002 | `(AUTOX-FR-005): signing key → working git repository (GITX-FR-002) → coder agent (roles + CLI)` |
| 268 | docstring | RUNX-FR-007 | `"""The additive executive-dispatch provenance recorded per dispatched stage (RUNX-FR-007).` |
| 271 | format-example | NFR-002 | `hash-chained ledger as the run's verdict, so altering one is detectable by ``3pwr verify`` (NFR-002)."""` |
...
## engine/src/threepowers/scaffold.py (58)
...
| 185 | echoed-message | ONBRD-NFR-004 | `"# None suppresses a mandatory human gate or weakens a threshold (ONBRD-NFR-004 / INITX-NFR-002).\n"` |
| 185 | echoed-message | INITX-NFR-002 | `"# None suppresses a mandatory human gate or weakens a threshold (ONBRD-NFR-004 / INITX-NFR-002).\n"` |
| 195 | comment | AGENTX-FR-017 | `# The explanatory header rewritten role files keep (AGENTX-FR-017): `require_dispatch` and the` |
| 205 | echoed-message | 3PWR-FR-021 | `"# (3PWR-FR-021, epic A3): when true, a High-risk `advance` refuses unless an ISOLATED\n"` |
| 211 | echoed-message | 3PWR-FR-022 | `"# Model diversity (oracle/reviewer vs coder) is RECOMMENDED, never forced (3PWR-FR-022): a\n"` |
| 214 | echoed-message | 3PWR-FR-057 | `"# (3PWR-FR-057).\n"` |
| 216 | echoed-message | 3PWR-FR-022 | `'# `diversity_level` (default `family`) is how "diverse enough" is judged (3PWR-FR-022):\n'` |
| 223 | echoed-message | EXEC-FR-015 | `"# (EXEC-FR-015/NFR-003) \u2014 set by `3pwr init`'s multi-select to the CLIs you have installed.\n"` |
| 223 | format-example | NFR-003 | `"# (EXEC-FR-015/NFR-003) \u2014 set by `3pwr init`'s multi-select to the CLIs you have installed.\n"` |
| 237 | docstring | INITX-FR-002 | `"""Record a role's concrete model + integration in ``roles.yaml`` (INITX-FR-002/003, AGENTX-FR-012).` |
| 237 | docstring | AGENTX-FR-012 | `"""Record a role's concrete model + integration in ``roles.yaml`` (INITX-FR-002/003, AGENTX-FR-012).` |
| 240 | docstring | AGENTX-NFR-003 | `other role and unrelated field (AGENTX-NFR-003). ``model_family`` wins when given (a catalog` |
| 241 | docstring | AGENTX-FR-015 | `entry's family — AGENTX-FR-015); otherwise the family is derived from the model id where it` |
| 243 | docstring | AGENTX-FR-012 | `(AGENTX-FR-012): an explicit value wins, an existing value is preserved, else the documented` |
| 244 | docstring | AGENTX-FR-017 | `default ``false``. The rewritten file keeps an explanatory header (AGENTX-FR-017)."""` |
| 276 | docstring | AGENTX-FR-017 | `"""Write roles.yaml with the explanatory header, preserving field order (AGENTX-FR-017)."""` |
| 286 | docstring | EXEC-FR-015 | `"""Record which agent-backend CLIs a live ``3pwr run`` may dispatch (EXEC-FR-015/NFR-003).` |
| 286 | format-example | NFR-003 | `"""Record which agent-backend CLIs a live ``3pwr run`` may dispatch (EXEC-FR-015/NFR-003).` |
| 289 | docstring | AGENTX-NFR-003 | `other field. An empty selection is a no-op — it never wipes the seeded default (AGENTX-NFR-003)."""` |
| 303 | docstring | 3PWR-FR-022 | `"""Record how model diversity is judged — ``family`` or ``model`` (3PWR-FR-022).` |
| 315 | comment | STEER-NFR-002 | `# comments on the first rewrite, and the secret-safety rule (STEER-NFR-002) is worth keeping WHERE` |
| 318 | echoed-message | STEER-FR-010 | `"# 3Powers run notifications (STEER-FR-010). `3pwr run` fires on gate pauses, failures, and\n"` |
| 320 | echoed-message | STEER-NFR-002 | `"# SECRETS ARE NEVER STORED HERE (STEER-NFR-002): a slack/teams block names an env var\n"` |
| 327 | docstring | STEER-FR-010 | `"""Record one run-notification channel in ``notifications.yaml`` (STEER-FR-010).` |
| 331 | docstring | STEER-NFR-002 | `value: slack/teams carry ``webhook_env`` and email ``password_env`` (STEER-NFR-002)."""` |
| 354 | docstring | ONBRD-NFR-001 | `"""True iff ``path`` resolves OUTSIDE the repository working tree (ONBRD-NFR-001 / SC-003)."""` |
| 355 | comment | HARDN-FR-002 | `from .keys import inside_working_tree  # single custody source of truth (HARDN-FR-002)` |
| 364 | docstring | ONBRD-FR-016 | `"""Write a 3Powers-flavoured AGENTS.md starter if the repo has none (ONBRD-FR-016).` |
| 372 | comment | DOCX-FR-004 | `# 3Powers-owned path (DOCX-FR-004; relocated out of the former Spec-Kit tree).` |
| 389 | docstring | ONBRD-FR-015 | `(ONBRD-FR-015) — a user-authored one is left alone."""` |
| 398 | docstring | ONBRD-FR-015 | `"""Lay the 3Powers constitution at ``.3powers/memory/constitution.md`` (ONBRD-FR-015, DOCX-FR-005).` |
| 398 | docstring | DOCX-FR-005 | `"""Lay the 3Powers constitution at ``.3powers/memory/constitution.md`` (ONBRD-FR-015, DOCX-FR-005).` |
| 414 | docstring | INITX-FR-010 | `"""True iff a recognized CI/CD configuration is present (INITX-FR-010).` |
| 417 | docstring | INITX-FR-010 | `It asserts *presence* only — never that the pipeline is complete or correct (INITX-FR-010)."""` |
| 448 | docstring | INITX-FR-011 | `"""True iff AGENTS.md exists and is still the unfilled 3Powers starter (INITX-FR-011).` |
| 460 | docstring | ONBRD-FR-015 | `"""A checklist of what a project needs to run the full agentic workflow (ONBRD-FR-015/016).` |
| 462 | docstring | INITX-FR-009 | `Extended for INITX-FR-009/010/011 with CI/CD presence and whether AGENTS.md is still an unfilled` |
| 475 | docstring | ONBRD-FR-007 | `"""Create the Ed25519 signer, refusing to overwrite an existing key unless forced (ONBRD-FR-007).` |

## engine/src/threepowers/scanners.py (13)
...
| 125 | echoed-message | HARDN-FR-003 | `q.findings.append("core ed25519-priv private-key check ran clean (HARDN-FR-003)")` |
...
## engine/src/threepowers/steering.py (22)
...
| 30 | echoed-message | 3PWR-FR-006 | `"review-spec": ("specify", "Spec"),  # the spec artifact under human review (3PWR-FR-006)` |
...
## engine/src/threepowers/style.py (41)
...
| 66 | echoed-message | TRIX-FR-008 | `_ANSI_RE = re.compile(r"\033\[[0-9;]*m")  # a strip matcher only, never construction (TRIX-FR-008)` |
...
## engine/src/threepowers/verdict.py (17)
...
| 28 | echoed-message | SLOCK-FR-004 | `"spec_integrity",  # the approved spec is unchanged — fails fast, before any test (SLOCK-FR-004)` |
| 37 | echoed-message | 3PWR-FR-008 | `"defect_regression",  # work-kind: defect (3PWR-FR-008)` |
| 38 | echoed-message | 3PWR-FR-009 | `"contract_check",  # work-kind: design — structural/API contract (3PWR-FR-009)` |
| 39 | echoed-message | 3PWR-FR-009 | `"component_contract",  # work-kind: design — component contract (3PWR-FR-009)` |
| 40 | echoed-message | 3PWR-FR-009 | `"a11y_scan",  # work-kind: design — accessibility (3PWR-FR-009)` |
| 41 | echoed-message | 3PWR-FR-009 | `"visual_regression",  # work-kind: design — visual regression (3PWR-FR-009)` |
