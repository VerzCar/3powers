...
| | |
|---|---|
| **Current milestone** | **v0.5 complete; v1.0 in progress** (spec §17 phasing; plans 001–027 delivered) |
...
> the requirement IDs below are the point. Last updated after **plan 027 (SRCX)**.
...
# engine dev loop
(cd engine && uv sync --extra dev && uv run pytest)          # 643 tests
...
plan/                       # the continuous plan series 001..027 (027 = SRCX: the flat run artifact workspace + completion gate)
```
