...
| | |
|---|---|
| **Current milestone** | **v0.5 complete; v1.0 in progress** (spec §17 phasing; plans 001–026 delivered) |
...
> the requirement IDs below are the point. Last updated after **plan 026 (AGENTX)**.
...
# engine dev loop
(cd engine && uv sync --extra dev && uv run pytest)          # 623 tests
...
plan/                       # the continuous plan series 001..026 (026 = AGENTX: stage agent templates + role setup)
```
