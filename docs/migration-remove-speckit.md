...
- `3pwr run` no longer shells out to `specify workflow run`. It uses the **native executive**: it dispatches
  each stage to the headless agent named by each role's `integration` in `.3powers/config/roles.yaml`,
  described by a manifest in `.3powers/agents/<name>.yaml`.
...
**Keep**: `.specify/memory/constitution.md` (your 3Powers constitution) and `.specify/templates/`
(spec/plan/tasks authoring templates) — these are engine-owned and still used.
