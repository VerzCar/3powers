---
description: "Phase A — author the independent oracle tests from a SEALED, spec-only bundle (3PWR-FR-020). Judiciary role; structurally separated from the coder, recorded and verified against the ledger."
handoffs:
...
- Place tests under the target's `tests/{unit,integration,e2e}/` to exercise all three layers
  (3PWR-FR-064). Author tests from the sealed criteria, not from any implementation.

### Finish

1. **Record the authoring (3PWR-FR-062):**
...
- [ ] `3pwr oracle record` succeeded with a model family different from the coder's (3PWR-FR-022).
