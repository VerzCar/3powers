"""Colorized, package-manager-style CLI output + a structured-output toolkit (INITX-FR-013/014, CLIUX, TRIX).
...
(3PWR-NFR-001, INITX-FR-014). Every ANSI escape this module emits is generated by ``rich`` style
primitives (TRIX-FR-002/008; CLIUX-FR-003 as amended) — no hand-rolled SGR construction — and the
styler degrades to a no-op when color is off (INITX-NFR-004 — its absence never fails a command).
No network is ever touched (CLIUX-NFR-003, TRIX-NFR-001).
...
# The semantic style vocabulary, each name a rich Style (TRIX-FR-002). rich renders the standard
...
def color_enabled(
    ...
    *,
    ...
    environment-layer fallback (TRIX, CLIUX-FR-003 as amended); then ``color_mode``
...
class Styler:
    ...
    structured-output toolkit (CLIUX-FR-001). Every escape byte is generated by ``rich``
    (TRIX-FR-002/008) — this class holds no SGR knowledge of its own.
    ...
    TRIX-FR-006). ``ascii_only`` swaps the Unicode glyphs for an ASCII set when the stream cannot
    encode them (CLIUX-NFR-004); it defaults off, so a directly-constructed ``Styler`` still uses
    the Unicode marks."""
    ...
    def paint(self, text: str, *names: str) -> str:
        ...
        returned unchanged otherwise or when no name maps (TRIX-FR-002)."""
