...
- A `3pwr` run's stage artifacts lie flat in the run's auto-allocated feature folder `specs-src/<NNN>-<slug>/` — `discovery.md` (when the conditional Discovery stage ran — feature/design work opens with it, defect/docs/chore/refactor skip it), `spec.md`, `plan.md`, `implementation-plan.md`, `oracle.md`, `changelog.md` (the agent-authored, non-validated Keep-a-Changelog release note for the run — informational, never gates; the top-level `CHANGELOG.md` stays hand-maintained), plus the engine-maintained `progress.md`. Stage prompts are assembled from markdown templates — repo-local `.3powers/templates/agents/*.agent.md` overrides the engine's bundled defaults; the templates are the single source of the dispatched instructions. The legacy artifact names (`tasks.md`, `implement.md`), the legacy base folder `specs/`, and the legacy split layout (`specs/<feature>/spec/spec.md` + the sibling `specs/<feature>/artifacts/` folder) stay readable.
- Implementation-plan artifacts group work into ordered phases sized to the context budget (`.3powers/config/context.yaml`); a phase marked `[P]` is dispatched in parallel as a fresh session only when its declared dependencies completed in a prior batch and its file scope is disjoint from every other phase in the batch — otherwise it is serialized with a named reason. A phased build commits one commit per implement phase (`implement(phase N/M): …`) plus a trailing record commit, and the engine commits its own state after each judgment step and before each human gate, so a finished run leaves a clean working tree. The budget is advisory — an oversize phase warns, never blocks. Every phase runs the coding gates over its file scope (advisory), and the final phase is always a dedicated Verification phase.
