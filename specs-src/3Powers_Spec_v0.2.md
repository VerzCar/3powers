...
| **AUTOX** (spec 014) | draft | Auto full-mode readiness + the run error contract: init readiness unified with the run preflight (one source — env-key validation, agent-CLI presence, honest auth caveat) plus a standalone readiness command; every run failure **recorded in the signed ledger** and surfaced by `--status`; per-attempt **transcripts** persisted with credential redaction; a documented **exit-code/JSON status contract**; failure-resume independent of auto-commit; an end-user getting-started + failure-class troubleshooting. |
